...
void Init(int argc, char** argv) {
	...
	Config::window = glutCreateWindow("Jens Eckert jeckert5");
...
void DrawAxes() {
	...
	if (SelectedObject != nullptr)
		glTranslatef(SelectedObject->position.x - MainScene.camera.position.x, SelectedObject->position.y - MainScene.camera.position.y, SelectedObject->position.z);
...
void SmoothInput() {
	if (GetAsyncKeyState('A') && !Config::PlayMode)
		MainScene.camera.position.x -= Time::deltaTime;

	if (GetAsyncKeyState('D') && !Config::PlayMode)
		MainScene.camera.position.x += Time::deltaTime;

	if (GetAsyncKeyState('W') && !Config::PlayMode)
		MainScene.camera.position.y += Time::deltaTime;
...
void KeyInput(unsigned char key, int x, int y) {
	...
	case 'a':
		...
		break;
	case 'g':
		MainScene.menu->active = false;
		MainScene.inMenu = false;
		break;
	case 'p':
		MainScene.paused = !MainScene.paused;
		MainScene.pause->active = !MainScene.pause->active;
		break;
	case 27:
...
void PlayButtonAction() {
	...
	if (Config::PlayMode) {
		...
		MainScene.camera.position = glm::vec3(0);
	}
...
void BuildUI() {
	...
	FileDropDown = Panel({ 42.5f, Config::WindowH - TOP_PANEL_HEIGHT - 18 }, { 50, 25 }, { 1, 1, 1 });
	...
	// File Dropdown
	Button exit(25, 12.5f, 50, 25, 70, 55, "Exit", Exit);
	...
	Button addGround(
		...
		50, 560, "",
		[&] {
			auto temp = Object::create("ground" + std::to_string(MainScene.ground.size() + 1), { Texture::create("textures/ground.png") });
			auto t2 = std::make_shared<Object>(temp);
			MainScene.children.emplace_back(t2);
			MainScene.ground.emplace_back(t2);

			auto s = MainScene.children.size() - 1;

			Button t(75, 465.f - (s * 35.f), LEFT_PANEL_WIDTH, 35, 70, 65.f + (s * 35.f), MainScene.children.back()->name, [&, s] {SelectedObject = MainScene.children[s]; SetButtonVals(); });
			t.text.scale = 0.3f;
	...
	Object g = Object::create("g", { Texture::create("textures/ground.png") });
	...
	Button addObstacle(
		...
		130, 560, "",
		[&] {
			auto temp = Object::create("obstacle" + std::to_string(MainScene.obstacles.size() + 1), { Texture::create("textures/obstacle.png") });
			auto balls = std::make_shared<Object>(temp);
			MainScene.children.emplace_back(balls);
			MainScene.obstacles.emplace_back(balls);

			auto s = MainScene.children.size() - 1;

			Button t(75, 465.f - (s * 35.f), LEFT_PANEL_WIDTH, 35, 70, 65.f + (s * 35.f), MainScene.children.back()->name,
					 [&, s] {
						 SelectedObject = MainScene.children[s];
						 SetButtonVals();
					 });
	...
	BottomPanel.addButton(addObstacle);

	Object o = Object::create("o", { Texture::create("textures/obstacle.png") });
	o.scale = glm::vec3(70, 70, 1);
	o.position.x = 130;
	o.position.y = 35;
	BottomPanel.objects.emplace_back(o);
	...
	Button addCoin(
		...
		[&] {
			...
			MainScene.coins.emplace_back(new Coin);
			auto coin = MainScene.coins.back();
			coin->name = "coin" + std::to_string( MainScene.coins.size());
			MainScene.children.emplace_back(coin);
	...
	Coin coin;
	coin.scale = glm::vec3(70, 70, 1);
	coin.position.x = 210;
	coin.position.y = 35;
	BottomPanel.objects.emplace_back(coin);
}
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < MainScene.children.size(); i++) {
		Button b(75, 465.f - (i * 35), LEFT_PANEL_WIDTH, 35, 70, 65.f + (i * 35.f), MainScene.children[i]->name, [&, i] { SelectedObject = MainScene.children[i]; SetButtonVals(); });
		b.text.scale = 0.3f;
